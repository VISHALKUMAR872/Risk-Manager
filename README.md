...
Historical features also introduce a major data leakage risk.



Risk Sentinel addresses this with point-in-time historical priors and transaction-time replay.
...
Estimate the probability that a transaction is fraudulent.
...
Combine the prediction with real-time velocity and entity-level context.
...
Translate fraud probability into expected financial loss.
...
Use a deterministic policy engine to determine the appropriate action.
...
train\_transaction.csv	\~651.69 MB

train\_identity.csv	\~25.3 MB

test\_transaction.csv	\~584.79 MB

test\_identity.csv	\~24.6 MB
...
Fraud probabilities are used downstream for expected-loss calculation, making probability calibration important.



Risk Sentinel uses isotonic regression.
...
while ROC-AUC remained essentially unchanged.
...
LOW\_FRICTION	64.92%	12.23%	0.2350%	0.6469%

BALANCED	41.54%	19.53%	0.9773%	1.6143%

MAX\_RECALL	41.24%	19.72%	0.9995%	1.6426%

Locked future evaluation

Policy	Precision	Recall	FPR	Intervention

LOW\_FRICTION	62.24%	12.62%	0.2760%	0.7056%
...
A controlled local benchmark processed:
...
A separate 200-event benchmark reported:



Average decision latency: \~97.9 ms
...
ELEVATED\_EXPECTED\_LOSS

LLM boundary



The LLM is not the authoritative fraud decision-maker.



LLM functionality is reserved for investigation/explanation workflows.



LLM output may assist an investigator, but it must not override the deterministic risk decision.
...
FastAPI + Pydantic provide typed API validation.



Production authentication and rate limiting remain hardening requirements.
...
Start FastAPI using the repository's configured ASGI module.
...
ml/artifacts/models/fraud\_online\_v2\_priors\_lightgbm.txt
...
ml/artifacts/models/fraud\_online\_v2\_priors\_lightgbm\_isotonic\_calibrator.joblib
...
├── v2\_lightgbm\_policy\_optimization.json

└── v2\_lightgbm\_policy\_frontier.csv
...
The production LightGBM artifact is approximately:
...
The current controlled benchmark demonstrates:



\~18 worker-completed transactions/sec
...
Risk Sentinel does not claim to eliminate fraud.



It provides a measurable, explainable and economically-aware framework for deciding how transactions should be handled under fraud risk.
